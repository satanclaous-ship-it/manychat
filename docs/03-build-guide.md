...
- [ ] **`satanclaous-ship-it/auto-dm` 빈 레포 생성** (GitHub에서 New repository → private → README 체크). 자동 생성이 연동 권한으로 막혀 이 한 번만 수동. 만든 뒤 이 문서 3종을 그 레포 `docs/`에 복사하고 오푸스 세션을 그 레포에서 연다.
